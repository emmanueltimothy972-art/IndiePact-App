...
type Stage = "verifying" | "success" | "already_handled" | "error";
      ...
      try {
        ...
        const res = await fetch(
          `${window.location.origin}${base}/api/subscription/verify-payment`,
          {
            ...
            body: JSON.stringify({ reference, planKey: "subscription" }),
          },
        );
        ...
        if (res.ok || res.status === 409) {
          ...
          setStage("error");
          setErrorMsg(body.error ?? "Payment verification failed. Contact support if your plan hasn't updated.");
          setTimeout(() => navigate(`${base}/dashboard`), 4000);
        }
      } catch {
        setStage("error");
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-800 bg-[#0a0a0a] p-8 flex flex-col items-center text-center gap-5">
        ...
        {stage === "error" && (
          <>
            ...
            <div>
              <p className="text-white font-semibold text-lg mb-1">Verification issue</p>
