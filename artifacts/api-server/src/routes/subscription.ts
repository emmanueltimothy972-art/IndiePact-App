...
// ─── NGN billing config ───────────────────────────────────────────────────────
// Mirrors scripts/paystack-plan-codes.json (generated by create-paystack-plans.js).
// Amounts are in kobo (₦1 = 100 kobo).
// USD MIGRATION: change currency to "USD", swap amounts to cents, update plan
// codes after Paystack enables multi-currency — no logic changes required.
...
const PLAN_AMOUNTS_NGN_KOBO: Record<string, number> = {
  starter:    3_000_000,   // ₦30,000  ≈ $19/mo
  pro:        8_000_000,   // ₦80,000  ≈ $49.99/mo
  business:   16_000_000,  // ₦160,000 ≈ $99/mo
  agency:     24_000_000,  // ₦240,000 ≈ $149/mo
  enterprise: 32_000_000,  // ₦320,000 ≈ $199/mo
};
...
router.post("/subscription/initialize", requireAuth, async (req, res) => {
  ...
  const amountKobo = PLAN_AMOUNTS_NGN_KOBO[tierName];

  if (!planCode || amountKobo === undefined) {
  ...
  req.log.info(
    { userId, email, tierName, planCode, event: "billing_init_start" },
    `[BILLING_INIT] Initiating subscription checkout session for user: ${email} on tier: ${tierName}`,
  );
  ...
  const payload = {
    ...
    metadata: {
      userId,
      tierName,
      planKey: tierName,  // kept for backward-compat with existing webhook handler
  ...
  if (!paystackRes.ok || paystackBody.status !== true || !paystackBody.data) {
    req.log.error(
      { userId, tierName, status: paystackRes.status, message: paystackBody.message, event: "paystack_init_rejected" },
      "Paystack returned an error during transaction initialization",
    );
    return res.status(500).json({
      error: "Payment processor rejected the request.",
      detail: paystackBody.message ?? `HTTP ${paystackRes.status}`,
    });
  }
